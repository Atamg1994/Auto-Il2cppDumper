...
// snity-shim:node_modules/@snity/base64-js/index.js
var lookup = [];
var revLookup = [];
...
// snity-shim:node_modules/@snity/ieee754/index.js
...
// snity-shim:node_modules/@snity/buffer/index.js
var config = {
...
// node_modules/snity-java-bridge/lib/android.js
var android_exports = {};
...
// node_modules/snity-java-bridge/lib/alloc.js
var {
  pageSize,
  pointerSize
} = Process;
...
// node_modules/snity-java-bridge/lib/result.js
var JNI_OK = 0;
...
// node_modules/snity-java-bridge/lib/jvmti.js
var jvmtiVersion = {
  v1_0: 805371904,
  v1_2: 805372416
};
...
// node_modules/snity-java-bridge/lib/machine-code.js
...
// node_modules/snity-java-bridge/lib/memoize.js
...
// node_modules/snity-java-bridge/lib/env.js
...
// node_modules/snity-java-bridge/lib/vm.js
...
function withAllArtThreadsSuspended(fn) {
  ...
  api2["art::ThreadList::SuspendAll"](threadList, Memory.allocUtf8String("snity"), longSuspend ? 1 : 0);
  try {
    fn();
  } finally {
...
function requestDeoptimization(vm3, env, kind, method) {
  ...
  withRunnableArtThread(vm3, env, (thread) => {
    if (getAndroidApiLevel() < 30) {
      ...
      switch (kind) {
        case kFullDeoptimization:
          api2["art::Instrumentation::DeoptimizeEverything"](instrumentation, Memory.allocUtf8String("snity"));
          break;
        case kSelectiveDeoptimization:
...
// node_modules/snity-java-bridge/lib/jvm.js
var jsizeSize2 = 4;
...
// node_modules/snity-java-bridge/lib/api.js
var getApi3 = getApi;
try {
  getAndroidVersion();
} catch (e) {
...
// node_modules/snity-java-bridge/lib/class-model.js
...
// node_modules/snity-java-bridge/lib/lru.js
var LRU = class {
...
// node_modules/snity-java-bridge/lib/mkdex.js
var kAccPublic2 = 1;
var kAccNative2 = 256;
...
// node_modules/snity-java-bridge/lib/types.js
...
// node_modules/snity-java-bridge/lib/class-factory.js
var jsizeSize3 = 4;
var {
...
// node_modules/snity-java-bridge/index.js
var jsizeSize4 = 4;
...
(function() {
    if (typeof Module.findExportByName !== 'undefined') return;

    console.log("[*] Frida 17+ detected. Applying compatibility polyfills...");

    // 1. Поиск экспортов
    Module.findExportByName = function (moduleName, exportName) {
        if (moduleName === null) return Module.findGlobalExportByName(exportName);
        try { return Process.getModuleByName(moduleName).findExportByName(exportName); } 
        catch (e) { return null; }
    };

    Module.getExportByName = function (moduleName, exportName) {
        if (moduleName === null) return Module.getGlobalExportByName(exportName);
        return Process.getModuleByName(moduleName).getExportByName(exportName);
    };

    // 2. Базовые адреса и работа с модулями
    Module.findBaseAddress = function (name) {
        try { return Process.getModuleByName(name).base; } 
        catch (e) { return null; }
    };

    Module.getBaseAddress = function (name) {
        return Process.getModuleByName(name).base;
    };

    // 3. Перечисления (Exports, Imports, Symbols, Ranges)
    Module.enumerateExports = function (name, callbacks) {
        return Process.getModuleByName(name).enumerateExports(callbacks);
    };

    Module.enumerateImports = function (name, callbacks) {
        return Process.getModuleByName(name).enumerateImports(callbacks);
    };

    Module.enumerateSymbols = function (name, callbacks) {
        return Process.getModuleByName(name).enumerateSymbols(callbacks);
    };

    Module.enumerateRanges = function (name, protection, callbacks) {
        return Process.getModuleByName(name).enumerateRanges(protection, callbacks);
    };

    // 4. Прочие утилиты
    Module.ensureInitialized = function (name) {
        return Process.getModuleByName(name).ensureInitialized();
    };

    // 5. Глобальные перечисления (переехали в Process)
    if (typeof Module.enumerateModules === 'undefined') {
        Module.enumerateModules = function (callbacks) {
            return Process.enumerateModules(callbacks);
        };
    }
...
function logError(msg) {
   Java.performNow(function() {
    try {
        ...
        var fos = FileOutputStream.$new(errorLog, true); // true = дозапись (append)
        var ps = PrintStream.$new(fos);
        ps.println("[" + new Date().toLocaleTimeString() + "] PID " + myPid + ": " + msg);
        ps.close();
    } catch (e) {
        console.error("Не удалось записать лог: " + e);
    }
     });
}
console.log = function (message) {
    ...
    var ModRoot = "/sdcard/Documents/snityMods"; // Базовая папка для всех модов
    var errorLog = "/sdcard/Documents/snity_error.log";
    var bridgeFile = "/sdcard/Documents/snity_bridge.txt";
    var myPid = Process.id;
    var packageName = "";
	// Определяем папку архитектуры
...
// СРАЗУ (без ожидания Java) блокируем нативные выходы
  ...
  function showToast(text) {
    Java.perform(function() {
      ...
      var runnable = Java.registerClass({
        name: "com.snity.toast.Runnable",
        implements: [Java.use("java.lang.Runnable")],
        methods: {
          run: function() {
            Toast.makeText(context, String2.$new(text), 1).show();
          }
        }
      });
  ...
  function showToast_rnd(text) {
    Java.perform(function() {
        try {
            var ActivityThread = Java.use("android.app.ActivityThread");
            var currentApplication = ActivityThread.currentApplication();
            if (!currentApplication) return; // Если вызвали слишком рано

            var context = currentApplication.getApplicationContext();
            var Toast = Java.use("android.widget.Toast");
            var String2 = Java.use("java.lang.String");
            
            // Генерируем уникальное имя для класса, чтобы GSpace не ругался
            var className = "com.snity.toast.Runnable_" + Math.random().toString(36).substring(7);
...
var ToastRunnable = null; // Глобальная ссылка на наш класс

function showToast_ccc(text) { //instance
    Java.perform(function() {
        try {
            var ActivityThread = Java.use("android.app.ActivityThread");
            ...
            // 1. Создаем класс ТОЛЬКО ОДИН РАЗ
            if (ToastRunnable === null) {
                var className = "com.mod.UniversalToast_" + Math.random().toString(36).substring(7);
                
                ToastRunnable = Java.registerClass({
                    name: className,
                    implements: [Java.use("java.lang.Runnable")],
                    fields: {
                    ...
                    methods: {
                        run: function() {
                            // Внутри run берем данные из полей этого объекта
                            Toast.makeText(this.ctx.value, JavaString.$new(this.message.value), 1).show();
                        }
                    }
                });
                console.log("[*] Toast Class Registered: " + className);
            }

            // 2. Создаем экземпляр, заполняем данные и отправляем в Handler
            var runnableInstance = ToastRunnable.$new();
            runnableInstance.message.value = text;
            runnableInstance.ctx.value = context;

            var mainHandler = Handler.$new(Looper.getMainLooper());
            mainHandler.post(runnableInstance);

        } catch (e) {
            console.error("Optimized Toast Error: " + e);
        }
    });
}
...
// Функция для безопасного копирования файла средствами Java
function copyToInternal(sourcePath) {
    try {
        var File = Java.use("java.io.File");
        var FileInputStream = Java.use("java.io.FileInputStream");
        var FileOutputStream = Java.use("java.io.FileOutputStream");
        
        var srcFile = File.$new(sourcePath);
        if (!srcFile.exists()) return null;

        var context = Java.use("android.app.ActivityThread").currentApplication().getApplicationContext();
        var cacheDir = context.getCacheDir().getAbsolutePath();
        var fileName = sourcePath.split('/').pop();
        var destPath = cacheDir + "/lib_" + myPid + "_" + fileName;

        var srcStream = FileInputStream.$new(srcFile);
        var dstStream = FileOutputStream.$new(destPath);
        
        var srcChannel = srcStream.getChannel();
        var dstChannel = dstStream.getChannel();

        // МАГИЯ СКОРОСТИ: Прямая передача на уровне системы
        var size = srcChannel.size();
        ...
        var outParamFile = File.$new(destPath);
        outParamFile.setReadable(true, false);
        outParamFile.setExecutable(true, false);

        return destPath;
    } catch (e) {
        logError("FastCopy Error: " + e.toString());
        return null;
    }
}
        ...
        try {
            ...
            if (files) {
                for (var i = 0; i < files.length; i++) {
                    var f = files[i];
                    var name = f.getName();
                    // Удаляем только наши временные либы
                    if (name.startsWith("lib_") && name.endsWith(".so")) {
                        f.delete();
                    }
                }
            }
			
			 logError("Cleaning cache directory...");
            var cacheFiles = cacheDir.listFiles();
            if (cacheFiles) {
                for (var i = 0; i < cacheFiles.length; i++) {
                    var f = cacheFiles[i];
                    try {
                        // Удаляем рекурсивно, если это папка, или просто файл
                        if (f.isDirectory()) {
                            // Для папок можно вызвать delete(), если они пустые, 
                            // но обычно в cache лежат только файлы либ
                            f.delete(); 
                        } else {
                            f.delete();
                        }
                    } catch (e) { /* Игнорим ошибки занятых файлов */ }
                }
                logError("Cache cleared successfully.");
            }
...
function initModSystem() {
    Java.perform(function() {
        try {
            ...
            showToast("snity Mod System Loaded! ["+myPid+"]:"+packageName);
            // 1. Определяем архитектуру (ARM / x86)
            var arch = Process.arch;
            var archFolder = (arch === 'arm64') ? "arm64-v8a" : 
                             (arch === 'arm')   ? "armeabi-v7a" : 
                             (arch === 'x64')   ? "x86_64" : 
                             (arch === 'ia32')  ? "x86" : "unknown_" + arch;

            var File = Java.use("java.io.File");
            var appModDir = File.$new(ModRoot + "/" + packageName);
            
            // Структура: /Documents/snityMods/com.pkg/lib/x86_64/
            var libDir = File.$new(appModDir.getAbsolutePath() + "/lib/" + archFolder);
            var patchDir = File.$new(appModDir.getAbsolutePath() + "/patch");

            // 2. Создаем папки (mkdirs создаст всю цепочку)
            if (!libDir.exists()) libDir.mkdirs();
            if (!patchDir.exists()) patchDir.mkdirs();

            logError("ModSystem Active: " + packageName + " | Arch: " + archFolder);

            // 3. Авто-загрузка библиотек (.so)
            var libs = libDir.listFiles();
            if (libs) {
                for (var i = 0; i < libs.length; i++) {
                    var file = libs[i];
                    if (file.getName().endsWith(".so")) {
                        var safePath = copyToInternal(file.getAbsolutePath());
                        if (safePath) {
                            try {
                                Module.load(safePath);
                                logError("[+] Loaded lib: " + file.getName());
                            } catch (e) {
                                logError("[-] Load failed (" + file.getName() + "): " + e);
                            }
                        }
                    }
                }
            ...
            // 4. Авто-загрузка патчей (.js)
            var patches = patchDir.listFiles();
            if (patches) {
                ...
                for (var j = 0; j < patches.length; j++) {
                    var pFile = patches[j];
                    if (pFile.getName().endsWith(".js")) {
                        try {
                            var br = BufferedReader.$new(FileReader.$new(pFile));
                            var line, code = "";
                            while ((line = br.readLine()) !== null) { code += line + "\n"; }
                            br.close();
                            
                            (1, eval)(code); 
                            logError("[+] Patch applied: " + pFile.getName());
                        } catch (e) {
                            logError("[-] Patch error (" + pFile.getName() + "): " + e);
                        }
                    }
                }
            }
        } catch (err) { logError("ModLoader Critical Error: " + err); }
    });
}

    // Твой текущий слушатель bridgeFile (оставляем для ручного управления из GG)
    ...
    try {
        var File = Java.use("java.io.File");
        var file = File.$new(bridgeFile);
        
        if (file.exists()) {
            var scanner = Java.use("java.util.Scanner").$new(file);
            var content = "";
            ...
            if (content && content.includes("|")) {
                ...
                var targetPid = parseInt(parts[0]);

                if (targetPid === myPid) {
                    file.delete();// сразу чистим чтобы не дублировался инжект
                    var libPath = parts[1] ? parts[1].trim() : "";
                    var funcName = parts[2] ? parts[2].trim() : "";
                    showToast("Попытка инжекта: " + libPath);
                    try {
                   var safePath = copyToInternal(libPath);
                    
                    if (safePath) {
                        showToast("[*] Либа скопирована в: " + safePath);
                        var inject_lib = Module.load(safePath);
                        showToast("Либа загружена успешно.");
                        //let address = Module.findGlobalExportByName(rawName);
	                    //Module.getGlobalExportByName("readlink")
	                   //Process.getModuleByName("libc.so").getExportByName("epoll_wait")
                        if (funcName) {
                            var funcPtr = inject_lib.findExportByName(funcName);
							  if (funcPtr === null) {
								funcPtr = inject_lib.findSymbolByName(funcName);
							  }
                            if (funcPtr) {
                                var nativeFunc = new NativeFunction(funcPtr, 'void', []);
                                nativeFunc();
                                showToast("Функция " + funcName + " вызвана.");
                            } else {
                                showToast("ОШИБКА: Функция " + funcName + " не найдена в экспортах!");
                            }
                        }
                    
                        
                   }else{
                        showToast("ОШИБКА copyToInternal не смог перенести библиотеку в рабочую область");
                        logError("ОШИБКА copyToInternal не смог перенести библиотеку в рабочую область");
				   }
                    } catch (innerErr) {
                        showToast("ОШИБКА ИНЖЕКТА");
                        logError("ОШИБКА ИНЖЕКТА: " + innerErr.toString());
                    }
                }
            }
        }
    } catch (e) {
        logError("КРИТИЧЕСКАЯ ОШИБКА ЦИКЛА: " + e.toString());
    }
}, 5000);
});
    }
    ...
    // Ждем готовности Java и запускаем
    var readyCheck = setInterval(function() {
    try{
        if (Java.available) {
        
        const libc = Process.getModuleByName("libc.so");
    const exportsList = ["exit", "abort", "kill", "raise"];

    exportsList.forEach(name => {
        const addr = libc.getExportByName(name);
        if (addr) {
            Interceptor.replace(addr, new NativeCallback(function () {
                // Если это GSpace — НЕ блокируем выход (даем завершиться)
                if (processName === "com.gspace.android") {
                    const original = new NativeFunction(addr, 'void', []);
                    original();
                    return;
                }

                console.log("[!] Попытка завершения через " + name + " заблокирована");
            }, 'void', []));
        }
    });
		    ...
		    // 1. ПОЛУЧАЕМ ПУТЬ К SD-КАРТЕ ЧЕРЕЗ СИСТЕМУ
		    /*
            var Environment = Java.use("android.os.Environment");
            var storagePath = Environment.getExternalStorageDirectory().getAbsolutePath();
            
            // 2. ФОРМИРУЕМ ПУТИ ДИНАМИЧЕСКИ
            ModRoot = storagePath + "/Documents/snityMods";
            errorLog = storagePath + "/Documents/snity_error.log";
            bridgeFile = storagePath + "/Documents/snity_bridge.txt";
*/
            logError("System Paths Initialized: " + ModRoot);					 
            clearInterval(readyCheck);
			clearOldTempFiles();// очиста кеша !!
            startBridgeListener(); // Твой старый слушатель для GG
            initModSystem();
            }
        } catch (e) {
        logError("КРИТИЧЕСКАЯ ОШИБКА ЦИКЛА: " + e.toString());
